...
from callbacks_rag import (  # noqa: E402 # isort:skipq
    get_checkpoint_callback, get_early_stopping_callback,
...
from utils_rag import (  # noqa: E402 # isort:skip
    calculate_exact_match, flatten_list, get_git_info, is_rag_model, lmap,
...
class CustomDDP(DDPPlugin):
    def init_ddp_connection(self, global_rank=None, world_size=None) -> None:
        ...
        if not torch.distributed.is_initialized():
            ...
            torch_distrib.init_process_group(self.torch_distributed_backend,
                                             rank=global_rank,
...
class GenerativeQAModule(BaseTransformer):
                ...
                decoder_input_ids = (torch.cat(
                    [
                        torch.tensor([[decoder_start_token_id]] *
                                     target_ids.shape[0]).to(target_ids),
                        target_ids
                    ],
                    dim=1,
                ) if target_ids.shape[0] < self.target_lens["train"] else
                                     generator._shift_right(target_ids))
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        save_path = self.output_dir.joinpath("checkpoint{}".format(
            self.step_count))
...
def main(args=None, model=None) -> GenerativeQAModule:
    ...
    trainer: pl.Trainer = generic_train(
        ...
        profiler="advanced" if args.profile else None,
    )
